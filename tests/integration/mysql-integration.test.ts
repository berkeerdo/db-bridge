...
describe('MySQL Integration Tests', () => {
  ...
  it('should perform CRUD operations', async () => {
    ...
    const selectResult = await adapter.query(
      'SELECT * FROM integration_test_users WHERE id = ?',
      [userId]
    );
    ...
    // Update
    await adapter.execute(
      'UPDATE integration_test_users SET age = ? WHERE id = ?',
      [31, userId]
    );
    ...
    const deleteResult = await adapter.execute(
      'DELETE FROM integration_test_users WHERE id = ?',
      [userId]
    );
  ...
  it('should handle prepared statements', async () => {
    ...
    try {
      ...
      // Clean up
      await adapter.execute(
        'DELETE FROM integration_test_users WHERE email LIKE ?',
        ['%@test.com']
      );
    } finally {
      await stmt.close();
